...
@FindName_Loop:
                ...
                lea     ALLYNAME_MAX_LENGTH(a0),a0
                dbf     d1,@FindName_Loop
@GetNameCounter:
                
                moveq   #ALLYNAME_CHARACTERS_COUNTER,d2
@LoadName_Loop:
...
@ClearCaravanItems_Loop:
                ...
                moveq   #0,d0
                move.b  d0,((CARAVAN_ITEMS_NUMBER-$1000000)).w ; number of items in caravan
...
GetBreakFlag:
                ...
                andi.l  #BREAK_MASK,d0
                divu.w  #8,d0           ; get the byte in which the flag is stored
                loadSavedDataAddress CARAVAN_BROKEN_TRACKER, a0
                addSavedByteOffset d0, a0           ; go to the concerned byte
                swap    d0
                moveq   #$FFFFFF80,d1
                lsr.b   d0,d1
                rts
...
@MemberStatus_Loop:
                ...
                move.w  d0,d1
                addi.w  #FLAG_ALLY00_JOIN,d1
                bsr.s   CheckFlag
                beq.w   @CheckNextMember
                move.b  d0,(a2)+
                addq.w  #1,d2
                move.w  d0,d1
                addi.w  #FLAG_COUNT_ALLYSLOTS,d1
                bsr.w   CheckFlag
                beq.s   @InReserve
                move.b  d0,(a3)+
                addq.w  #1,d3
